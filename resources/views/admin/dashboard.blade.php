    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <x-card class="bg-white">
            <div class="flex items-center justify-between">
                <div><p class="text-sm text-slate-500">Total Guru</p><p class="text-2xl font-bold text-accent mt-1">{{ $stats['guru_count'] ?? 0 }}</p></div>
                <div class="w-12 h-12 bg-accent-soft rounded-xl flex items-center justify-center"><i class="ti ti-users text-xl text-accent"></i></div>
            </div>
        </x-card>
        <x-card class="bg-white">
            <div class="flex items-center justify-between">
                <div><p class="text-sm text-slate-500">Total Siswa</p><p class="text-2xl font-bold text-accent mt-1">{{ $stats['siswa_count'] ?? 0 }}</p></div>
                <div class="w-12 h-12 bg-emerald-50 rounded-xl flex items-center justify-center"><i class="ti ti-user text-xl text-emerald-600"></i></div>
            </div>
        </x-card>
        <x-card class="bg-white">
            <div class="flex items-center justify-between">
                <div><p class="text-sm text-slate-500">Total Kelas</p><p class="text-2xl font-bold text-accent mt-1">{{ $stats['kelas_count'] ?? 0 }}</p></div>
                <div class="w-12 h-12 bg-amber-50 rounded-xl flex items-center justify-center"><i class="ti ti-school text-xl text-amber-600"></i></div>
            </div>
        </x-card>
        <x-card class="bg-white">
            <div class="flex items-center justify-between">
                <div><p class="text-sm text-slate-500">Mata Pelajaran</p><p class="text-2xl font-bold text-accent mt-1">{{ $stats['mapel_count'] ?? 0 }}</p></div>
                <div class="w-12 h-12 bg-violet-50 rounded-xl flex items-center justify-center"><i class="ti ti-book text-xl text-violet-600"></i></div>
            </div>
        </x-card>
    </div>
